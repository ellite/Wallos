...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (
        ...
        if ($result === false) {
            ...
            $count = $row[0];
            if ($count == 0) {
                $query = "INSERT INTO webhook_notifications (enabled, url, headers, payload, user_id, ignore_ssl)
                              VALUES (:enabled, :url, :headers, :payload, :userId, :ignore_ssl)";
            } else {
                $query = "UPDATE webhook_notifications
                              SET enabled = :enabled, url = :url, headers = :headers, payload = :payload, ignore_ssl = :ignore_ssl WHERE user_id = :userId";
            }
