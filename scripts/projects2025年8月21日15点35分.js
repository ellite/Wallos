 ...
 */
/**
 * 【项目页面专用】根据当前过滤器重新从后端获取并刷新项目列表。
 * @param {string} initiator - 触发刷新的原因。
 */
function fetchProjects(initiator) {
  const container = document.querySelector("#subscriptions"); // 列表容器的ID
  if (!container) return;
  ...
  if (activeFilters['categories'] && activeFilters['categories'].length > 0) {
    getProjectsUrl += `?categories=${activeFilters['categories'].join(',')}`;
  }

  // (可以根据需要添加其他项目相关的过滤器URL参数)
  ...
  fetch(getProjectsUrl)
      ...
      .catch(error => {
        console.error('重新加载项目列表时出错:', error);
      });
}
/**
 * 【订阅页面专用】根据当前过滤器重新从后端获取并刷新订阅列表。
...
function fetchSubscriptions(id, event, initiator) {
  ...
  // 关键：确保指向获取订阅列表的正确后端接口
  let getSubscriptionsUrl = "endpoints/subscriptions/get.php";

  // 根据当前激活的过滤器拼接URL (这里是完整的订阅过滤器逻辑)
  if (activeFilters['categories'] && activeFilters['categories'].length > 0) {
    getSubscriptionsUrl += `?categories=${activeFilters['categories'].join(',')}`;
  }
  if (activeFilters['members'] && activeFilters['members'].length > 0) {
    getSubscriptionsUrl += getSubscriptionsUrl.includes("?") ? `&members=${activeFilters['members'].join(',')}` : `?members=${activeFilters['members'].join(',')}`;
  }
  if (activeFilters['payments'] && activeFilters['payments'].length > 0) {
    getSubscriptionsUrl += getSubscriptionsUrl.includes("?") ? `&payments=${activeFilters['payments'].join(',')}` : `?payments=${activeFilters['payments'].join(',')}`;
  }
  if (activeFilters['state'] && activeFilters['state'] !== "") {
    getSubscriptionsUrl += getSubscriptionsUrl.includes("?") ? `&state=${activeFilters['state']}` : `?state=${activeFilters['state']}`;
  }

  fetch(getSubscriptionsUrl)
      .then(response => response.text())
      .then(data => {
        if (data) {
...
document.querySelectorAll('.filter-item').forEach(function (item) {
  item.addEventListener('click', function (e) {
    ...
    // 只处理项目相关的 "categoryid" 过滤器
    if (this.hasAttribute('data-categoryid')) {
      const categoryId = this.getAttribute('data-categoryid');
      if (activeFilters.categories.includes(categoryId)) {
        activeFilters.categories.splice(activeFilters.categories.indexOf(categoryId), 1);
...
// --- 在全局点击事件中，加入关闭过滤器菜单的逻辑 ---
// (这段代码通常在 document.addEventListener('click',...) 中)
...
function openEditProject(event, projectId) {
  ...
  fetch(url)
      ...
      .then(data => {
        if (data && !data.error) {
          fillProjectEditForm(data);
          openAddProject();
        } else {
          showErrorMessage('加载项目信息失败: ' + (data.error || '未知错误'));
          // 如果加载失败，恢复页面滚动
          document.body.classList.remove('no-scroll');
        }
      })
